...
export default function Register() {
    ...
    // ✅ data.username = CONFIRMED claimed username only
    ...
    // ✅ claimInput = what user types in claim step (draft)
    const [claimInput, setClaimInput] = useState('');

    // Claim step should show if username is NOT confirmed, or if user forced edit
    ...
    // ✅ On load: if we already have a pending/claimed username, treat it as confirmed
    useEffect(() => {
        try {
    ...
    const claimLinkContinue = async (e) => {
        ...
        try {
            ...
            // ✅ CONFIRM the username only AFTER successful claim check
            setData((d) => ({ ...d, username: cleaned }));
            setClaimInput(cleaned);
            setForceClaimStep(false);

            // Persist for OAuth flow (register -> oauth -> auto-claim)
            try {
    ...
    const startOAuth = (provider) => {
        ...
        // ✅ mark this as register flow + store pending username (confirmed or draft)
        try {
                    ...
                    {verificationStep ? (
                        ...
                        <>
                            ...
                            <form onSubmit={claimLinkContinue} className="kc-form kc-form-claim">
                                ...
                                <button className="kc-btn kc-btn-primary kc-btn-center">
                                    Claim Link
                                </button>
                        ...
                        <>
                            ...
                            <p className="kc-bottom-line" style={{ marginTop: 18 }}>
                                Your link:{' '}
                                <strong>konarcard.com/u/{data.username}</strong>{' '}
                                <button
                                    type="button"
                                    className="kc-link"
                                    onClick={() => {
